...
/usr/bin/osascript -e 'display notification "Opening Whinfell Daily AM…" with title "Whinfell"' 2>/dev/null || true

echo "" >>"$LOG"
echo "=== AM command launch $(date) ===" >>"$LOG"
...
/usr/bin/python3 Whinfell_Daily_Launcher.py 2>>"$LOG"
CODE=$?
echo "exit=$CODE" >>"$LOG"
if [[ $CODE -ne 0 ]]; then
  /usr/bin/osascript -e "display alert \"Whinfell AM failed (exit $CODE)\" message \"See Desktop/whinfell_launcher.log\""
